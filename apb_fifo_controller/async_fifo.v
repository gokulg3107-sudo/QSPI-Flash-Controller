module apb_slave(rx_fifo_data, psel, penable, paddr, pwdata, pwrite, pready, pslverr, cmd_reg, addr_reg, dummy_reg, pclk, presetn, start, abort, width_reg, len_reg, tx_fifo_data, ctrl_reg, fifo_state_reg);
...
always@(current_state, psel, penable, count_access)begin
	...
	access: next_state = (count_access == 2) ? (psel & penable) ? setup : idle : idle;
...
always@(posedge pclk or negedge presetn)begin
	...
	else if (current_state == access) count_access <= count_access + 1;
	else count_access <= 0;
end
...
always@(posedge pclk or negedge presetn)begin
	if(!presetn) begin
		...
		if(current_state == access & count_access == 1 & pwrite) begin
			case(paddr) 
