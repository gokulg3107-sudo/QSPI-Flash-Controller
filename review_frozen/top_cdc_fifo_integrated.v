...
module top_cdc_fifo_integrated(sclk, psel, penable, pclk, presetn, pwrite, paddr, pwdata, pready, pslverr, cmd_reg_sync, len_reg_sync, dummy_reg_sync, addr_reg_sync, start_toggle, start_out, busy_reg, rx_read_data, qspi_busy_status, qspi_done_status, cs, si, so, wp, sio3);
...
fifo_read_fsm tx_read_logic(sclk, presetn, cmd_reg_sync, len_reg_sync, start_out, tx_data_req, tx_ren);

qspi_flash_controller qspi_flash(sclk, presetn, cmd_reg_sync, addr_reg_sync, dummy_reg_sync, len_reg_sync, fifo_read, start_out, cs, si, so, wp, sio3, dout, valid, tx_data_req);
...
rx_fifo_read_logic rx_read_logic(pclk, presetn, qspi_done_status, rx_ren, rx_read_data, rx_fifo_read_data, cmd_reg, len_reg);
