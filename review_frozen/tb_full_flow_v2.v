...
initial begin
    ...
    load_tx_byte(8'h17);
    load_tx_byte(8'h18);
    start_transaction(`page_program, 24'h000010, 5'd0, 8'd 20);
    wait_for_idle;
    ...
    start_transaction(`write_enable, 24'h0, 5'd0, 8'd0);
    wait_for_idle;
    start_transaction(`read_status_reg, 24'h0, 5'd0, 8'd0);
    wait_for_idle;
    

    $display("[%0t] ---- FAST READ (4 bytes @ 0x000010, 8 dummy cycles) ----", $time);
    start_transaction(`fast_read, 24'h000010, 5'd8, 8'd20);
    wait_for_idle;
...
initial begin
    $monitor("[%0t] flash WIP=%b WEL=%b", $time, flash_inst.WIP, flash_inst.WEL);
end
endmodule