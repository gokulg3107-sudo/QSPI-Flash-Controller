module domain_crossing(cmd_reg, addr_reg, dummy_reg, len_reg, start, start_out, sclk, presetn, busy_reg, cmd_reg_sync, addr_reg_sync, dummy_reg_sync, len_reg_sync, controller_busy );
input sclk, presetn, start;
input [7:0] cmd_reg, len_reg;
input [23:0] addr_reg;
input [4:0] dummy_reg;
input controller_busy;
...
reg  start_sync, start_prev, start_sync1, start_sync2, start_sync3;
...
always@(*)begin
        ...
        send_acknowledge: begin busy_temp = 0; start_out = 1; end
...
// busy_reg must stay asserted for the whole transaction, not just the
// one-cycle latch_inputdata handshake pulse. busy_temp covers the CDC
// handshake window (latch_inputdata/send_acknowledge); controller_busy
// covers the sequencer actually shifting data out on sclk afterward.
// Both are combinational on the sclk side and sampled on the same edge,
// so there's no gap between send_acknowledge dropping and
// qspi_flash_controller's current_state moving off idle.
always@(posedge sclk or negedge presetn)begin
        if(~presetn) busy_reg <= 0;
        else busy_reg <= busy_temp | controller_busy;
end
endmodule
